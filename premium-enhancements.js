(() => {
  ...
  css.textContent = `
    ...
    .mtgx-cursor-spotlight{position:fixed;z-index:999;pointer-events:none;width:320px;height:320px;border-radius:50%;transform:translate(-50%,-50%);background:radial-gradient(circle,rgba(161,205,255,.11),rgba(89,139,202,.035) 34%,transparent 70%);mix-blend-mode:screen;opacity:0;transition:opacity .25s ease}
    ...
    @media(max-width:700px){.mtgx-cursor-spotlight{display:none}.mtgx-sheen:hover:after{display:none}}
  `;
  document.head.appendChild(css);
  const vignette = document.createElement('div'); vignette.className='mtgx-dark-vignette'; document.body.appendChild(vignette);
  const spotlight = document.createElement('div'); spotlight.className='mtgx-cursor-spotlight'; document.body.appendChild(spotlight);
