(() => {
  ...
  style.textContent = `
    ...
    .mtgx-card-spotlight:hover .mtgx-card-edge{opacity:1}
    @media(prefers-reduced-motion:reduce){.mtgx-image-reveal{clip-path:none;transform:none;filter:none}.mtgx-card-spotlight:before{display:none}}
  `;
  ...
  const setup=()=>{
    document.querySelectorAll('.product-card,.notice-card,.portfolio-card,.channel-card,.hero-card,.quick-card').forEach(card=>{
      if(card.dataset.mtgxMicro)return;card.dataset.mtgxMicro='1';card.classList.add('mtgx-card-spotlight');const edge=document.createElement('i');edge.className='mtgx-card-edge';card.appendChild(edge);card.addEventListener('pointermove',e=>{const r=card.getBoundingClientRect();card.style.setProperty('--spot-x',`${e.clientX-r.left}px`);card.style.setProperty('--spot-y',`${e.clientY-r.top}px`)},{passive:true});
    });
